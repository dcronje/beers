...
class LocationHelper
{
    ...
    {
        return [
            '_id' => ['name' => '_id', 'type' => Type::nonNull(Type::int())],
        ];
    }
    ...
    {
        $location = Location::where('_id', '=', $args['_id'])->first();
        if (!$location) {
            throw new Exception('Location with _id: '.$args['_id'].' not found!', 404);
        }
        return $location;
    }
        ...
        if (isset($args['filters']) && !empty($args['filters'])) {
            if (isset($args['filters']) && !empty($args['filters'])) {
                if (isset($args['filters']['countries']) && !empty($args['filters']['countries'])) {
                    $qry->whereIn('country_id', $args['filters']['countries']);
                }
                if (isset($args['filters']['regions']) && !empty($args['filters']['regions'])) {
                    $qry->whereIn('region_id', $args['filters']['regions']);
                }
                if (isset($args['filters']['cities']) && !empty($args['filters']['cities'])) {
                    $qry->whereIn('city_id', $args['filters']['cities']);
                }
                if (isset($args['filters']['areas']) && !empty($args['filters']['areas'])) {
                    $qry->whereIn('area_id', $args['filters']['areas']);
                }
